...
//if (COM_NN_D(vector_1, vector_2) == 2) 
	...
	//	vector<int> number = { 0 };               //ýòî öåëàÿ ÷àñòü ÷èñëà ïðè äåëåíèè 2ãî íà 1å

	//	while (COM_NN_D(vector_1, vector_2) == 2) { //Äàëåå èäåò ïîäñ÷åò öåëîãî è äîïîëíåííàÿ ôóíêöèÿ Ìàêñà, òàê êàê èíà÷å íèõðåíà íå ïîëó÷àëîñü // äî $$$$$
	...
	//		while (b[0] == 0) // À âîò òóò äîïîëíåíèå: Åñëè ïîñëå âû÷åòàíèÿ â íà÷àëå âåêòîðà îñòàþòÿ íóëè, òî èõ  íóæíî óáðàòü 
	//			b.erase(b.begin());

	//		number = ADD_1N_N(number); // óâåëè÷èâàåì öåëîå
	//		vector_1 = result;
	//	}
	//	if (COM_NN_D(vector_1, vector_2) == 0) // åñëè äåëÿòñÿ íàöåëî, òî óâåëè÷èâàåì âåêòîð åùå íà 1öó
	//		number = ADD_1N_N(number);

	//	for (int i = 1; i < number.size(); i++) //Òàê êàê ìû ñ÷èòàåì äî 1é çíà÷àùåé öèôðû òî îñòàëüíîå çàìåíÿåì íóëÿìè
	//		number[i] = 0;
