#include "Allfunc.h"
vector<int>  SUB_NN_N(vector<int> a, vector<int> b) // ≈рмольев ћаксим ¬икторович
{	            
	...
	if (COM_NN_D(a, b) == 0)
	{
		vector <int> result = { 0 }; // “ак как натуральные числа равны, то их разность дает 0
		return result;          
	}
	if (COM_NN_D(a, b) == 1)
	{
		vector <int> result(b.size());                         //—озданим вектор, который хранит резльтат вычитани€, и назначим ему размерность большего числа
